...
function Week({workouts}:{workouts:Workout[]}){const[offset,setOffset]=useState(0),{monday,sunday}=weekBounds(offset),from=iso(monday),to=iso(sunday),runs=workouts.filter(w=>w.workout_date>=from&&w.workout_date<=to),planned=runs.reduce((s,w)=>s+Number(w.planned_distance_km||0),0),actual=runs.filter(w=>w.completed).reduce((s,w)=>s+Number(w.actual_distance_km||0),0),pct=planned?Math.min(100,Math.round(actual/planned*100)):0,days=Array.from({length:7},(_,i)=>{const d=new Date(monday);d.setDate(monday.getDate()+i);return d});return <><div className="week-head"><button className="icon-btn" onClick={()=>setOffset(o=>o-1)}><ChevronLeft size={18}/></button><div style={{textAlign:"center"}}><div className="eyebrow">WEEK</div><h2>{monday.getDate()} {monday.toLocaleString("en",{month:"short"})} – {sunday.getDate()} {sunday.toLocaleString("en",{month:"short"})}</h2></div><button className="icon-btn" onClick={()=>setOffset(o=>o+1)}><ChevronRight size={18}/></button></div><div className="stats section"><div className="stat"><span className="muted">Planned</span><strong>{km(planned)}</strong></div><div className="stat"><span className="muted">Actual</span><strong>{km(actual)}</strong></div></div><div className="progress"><span style={{width:`${pct}%`}}/></div><div className="week-strip">{days.map(d=>{const date=iso(d),dist=runs.filter(w=>w.workout_date===date).reduce((s,w)=>s+Number(w.planned_distance_km||w.actual_distance_km||0),0);return <div key={date} className={`day ${date===iso()?"today":""}`}>{d.toLocaleString("en",{weekday:"narrow"})}<b>{d.getDate()}</b><small>{dist?`${dist.toFixed(1)}k`:"–"}</small></div>})}</div>{runs.length?runs.map(w=><div className="card" key={w.id}><div className="title">{w.title}</div><div className="muted">{fmt(w.workout_date)}</div></div>):<div className="card empty"><p>Nothing scheduled</p></div>}</>}
function Plan({workouts,onComplete}:{workouts:Workout[];onComplete:(w:Workout)=>void}){const grouped=useMemo(()=>{const m=new Map<number,Workout[]>();workouts.filter(w=>w.source!=="manual").forEach(w=>{const k=w.week_number||0;m.set(k,[...(m.get(k)||[]),w])});return [...m.entries()].sort((a,b)=>a[0]-b[0])},[workouts]);const planned=workouts.reduce((s,w)=>s+Number(w.planned_distance_km||0),0);return <><div className="eyebrow">FULL PLAN</div><h1>{workouts.length} workouts · {km(planned)} planned</h1>{grouped.length?grouped.map(([week,runs])=><section className="section" key={week}><div className="week-head"><div className="eyebrow">{week?`WEEK ${week}`:"UNASSIGNED"}</div><b>{km(runs.reduce((s,w)=>s+Number(w.planned_distance_km||0),0))}</b></div>{runs.map(w=><WorkoutCard key={w.id} w={w} onComplete={onComplete} showDate/>)}</section>):<div className="card empty"><ClipboardList size={34}/><h2>No plan imported</h2><p className="muted">Open Settings to import your CSV plan.</p></div>}</>}
